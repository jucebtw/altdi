#!/bin/bash
# Скрипт для создания резервной копии проекта Витрина мастеров Алтая
# Запускать на сервере: bash backup.sh
...
cp -r /var/www/vitrina/backend/src "${BACKUP_PATH}/backend/"
cp -r /var/www/vitrina/backend/prisma "${BACKUP_PATH}/backend/"
...
cp -r /var/www/vitrina/frontend/src "${BACKUP_PATH}/frontend/"
...
echo "  # Затем скопируйте файлы обратно"
